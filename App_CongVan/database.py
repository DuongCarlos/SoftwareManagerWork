...
class Database:
    ...
    def create_table(self):
        ...
        # 3. Nâng cấp Bảng việc định kỳ (Thêm File đính kèm, Thời gian, Deadline)
    ...
    # ====== CÁC HÀM CÔNG VĂN CHÍNH ======
    def insert_cong_van(self, data):
        self.cursor.execute('''
            INSERT INTO cong_van (parent_id, ten_cong_viec, noi_dung, tai_lieu_dinh_kem, nguoi_xu_ly, nguoi_trinh_ky, ngay_nhan, ngay_bat_dau, ngay_ket_thuc, ngay_hoan_thanh, trang_thai, file_path)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', data)
        self.conn.commit()
    ...
    def update_cong_van(self, record_id, data):
        self.cursor.execute('''
            UPDATE cong_van 
            SET parent_id=?, ten_cong_viec=?, noi_dung=?, tai_lieu_dinh_kem=?, nguoi_xu_ly=?, nguoi_trinh_ky=?, ngay_nhan=?, ngay_bat_dau=?, ngay_ket_thuc=?, ngay_hoan_thanh=?, trang_thai=?, file_path=?
            WHERE id=?
        ''', (*data, record_id))
        self.conn.commit()
